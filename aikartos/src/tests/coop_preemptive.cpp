...
namespace tests {
	...
	int coop_preemptive(void)
	{
		...
		kernel::add_task(&task0, tasks::config{}.set<flags::quanta>(20));
		kernel::add_task(&task1, tasks::config{}.set<flags::quanta>(30));
		kernel::add_task(&task2, tasks::config{}.set<flags::quanta>(40));
		kernel::add_task(&task3, tasks::config{}.set<flags::quanta>(0)); // 0 == INFINITE
