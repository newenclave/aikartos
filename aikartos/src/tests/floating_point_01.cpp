...
#ifdef ENABLE_TEST_floating_pount_01
...
namespace {
	...
	void draw2(void*) {
		this_task::enable_fpu();
		rnd::xorshift32 rng(kernel::get_tick_count());
	    ...
	    con.clear();
	    while (true) {
	         float delta = ((rng.next() % 100) / 100.0f - 0.5f) * 1.9f;
	         angle += delta;
